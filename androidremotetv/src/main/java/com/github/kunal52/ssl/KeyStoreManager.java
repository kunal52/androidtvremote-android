...
//    static final char[] KEYSTORE_PASSWORD = "KeyStore_Password".toCharArray();
    ...
    private KeyStore load() {
        ...
        try {
            if (!useAndroidKeyStore()) {
                keyStore = KeyStore.getInstance(KeyStore.getDefaultType());
                keyStore.load(mContext.openFileInput(androidRemoteContext.getKeyStoreFileName()), androidRemoteContext.getKeyStorePass());
            } else {
                keyStore = KeyStore.getInstance(ANDROID_KEYSTORE);
                keyStore.load(null);
            }
    ...
    private void store(KeyStore keyStore) {
        if (!useAndroidKeyStore()) {
            try {
                FileOutputStream openFileOutput = new FileOutputStream(androidRemoteContext.getKeyStoreFileName());
